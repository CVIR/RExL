...
class RLExplainer(nn.Module):
    def __init__(self, model, state_generator, policy, action_space=7, mode='insertion', blur=None, average_maps=1, all_class=False):
        ...
        self.blur = blur
        self.HW = 224
        self.scale = int(self.HW/self.action_space)
    ...
    def forward(self, x_):
        ...
        for i in tqdm(range(self.average_maps), desc='Average:'):
            ...
            if self.mode == 'insertion':
                x = x_.clone().detach()
                x_canvas = x_blur_.clone().detach()
            else:
                x_canvas = x_.clone().detach()
                x = x_blur_.clone().detach()
                predictions = self.softmax(self.model(x_)).cpu().numpy()
